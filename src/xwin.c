...
xwin *init_xwin() {
    ...
    if (cfg.transparency < 1.0) {
        uint32_t cardinal_alpha =
            (uint32_t)(cfg.transparency * (float)((uint32_t)-1));
...
void initWindow(xwin *win) {
    int attr[] = {GLX_X_RENDERABLE, True, GLX_DRAWABLE_TYPE, GLX_WINDOW_BIT,
                  GLX_RENDER_TYPE, GLX_RGBA_BIT, GLX_X_VISUAL_TYPE,
                  GLX_TRUE_COLOR, GLX_RED_SIZE, 8, GLX_GREEN_SIZE, 8,
    ...
    for (int i = 0; i < elemc; i++) {
        win->vi =
            (XVisualInfo *)glXGetVisualFromFBConfig(win->display, win->fbcs[i]);
        if (!win->vi)
            continue;
    ...
    win->swa.colormap =
        XCreateColormap(win->display, win->root, win->vi->visual, AllocNone);
    win->swa.event_mask = StructureNotifyMask | ExposureMask | KeyPressMask;
    unsigned long mask = CWBackPixel | CWBorderPixel | CWColormap | CWEventMask;

    if (cfg.debug)
        printf("Window depth %d, %dx%d\n", win->vi->depth, win->width,
               win->height);

    win->window = XCreateWindow(win->display, win->root, win->offX, win->offY,
                                win->width, win->height, 0, win->vi->depth,
                                InputOutput, win->vi->visual, mask, &win->swa);
}
...
void initBackground(xwin *win) {
    ...
    unsigned long mask = CWOverrideRedirect | CWBackingStore | CWBackPixel |
